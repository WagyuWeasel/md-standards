---
title: "[MEETING NAME] — [DATE]"
type: meeting-notes
version: "1.0.0"
created: 2026-01-01
updated: 2026-01-01
author: ""
status: draft
tags: []
---

<!-- TEMPLATE: MEETING NOTES
     PURPOSE: Capture decisions, action items, and key discussion points
     so attendees and non-attendees can understand what happened and
     what comes next.
     AUDIENCE: Meeting attendees and anyone who needs to know the outcomes.
     SOURCE: Anthropic's prose-first style and Amazon's six-page memo
     discipline (decisions and actions over raw notes).
     KEY RULE: Prioritize decisions and action items over transcript-style
     notes. Nobody reads a wall of "X said Y then Z said W." -->

# [MEETING NAME] — [DATE]

**Date:** [YYYY-MM-DD]
**Time:** [START TIME] – [END TIME] [TIMEZONE]
**Location:** [ROOM / VIDEO LINK]
**Facilitator:** [NAME]
**Note-taker:** [NAME]

## Attendees

- [NAME] — [ROLE/TEAM]
- [NAME] — [ROLE/TEAM]
- [NAME] — [ROLE/TEAM]

**Absent:** [NAME], [NAME]

## Agenda

<!-- List what was planned for discussion. Check off items that were
     covered. -->

- [x] [TOPIC 1]
- [x] [TOPIC 2]
- [ ] [TOPIC 3 — carried over to next meeting]

## Decisions

<!-- The most important section. What was decided? Be explicit. -->

1. **[DECISION]:** [DETAILS — what was agreed, any conditions]
2. **[DECISION]:** [DETAILS]

## Action Items

<!-- Every action has an owner and a deadline. No orphan actions. -->

| Action | Owner | Deadline |
|:-------|:------|:---------|
| [WHAT NEEDS TO BE DONE] | [NAME] | [DATE] |
| [WHAT NEEDS TO BE DONE] | [NAME] | [DATE] |
| [WHAT NEEDS TO BE DONE] | [NAME] | [DATE] |

## Discussion Notes

<!-- Brief summary of key points discussed. Focus on context that
     explains the decisions above, not a transcript. -->

### [TOPIC 1]

[KEY POINTS DISCUSSED — 2-4 sentences summarizing the conversation,
disagreements, and how the group reached a conclusion.]

### [TOPIC 2]

[KEY POINTS DISCUSSED.]

## Parking Lot

<!-- Topics that came up but were deferred. Track them so they don't
     get lost. -->

- [DEFERRED TOPIC — to be discussed on [DATE] or in [FORUM]]
- [DEFERRED TOPIC]

## Next Meeting

**Date:** [DATE]
**Agenda preview:** [TOPICS EXPECTED]
